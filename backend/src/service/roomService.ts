...
export class RoomService {
  ...
  public canUpdateRoom(room: Room | null, userId: string): Promise<boolean> {
    return Promise.resolve(room !== null && room.userId === userId);
  }

  public canDeleteRoom(room: Room | null, userId: string): Promise<boolean> {
    return Promise.resolve(room !== null && room.userId === userId);
  }

  public canViewRoom(room: Room | null, userId: string): Promise<boolean> {
    return Promise.resolve(room !== null && room.userId === userId);
  }

  public async createRoom(name: string, userId: string): Promise<Room> {
    const room = new Room(undefined, name, userId);
