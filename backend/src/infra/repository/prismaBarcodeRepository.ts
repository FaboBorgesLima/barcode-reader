...
export class PrismaBarcodeRepository implements BarcodeRepository {
    ...
    private map(row: { id: string; value: string; roomId: string; quantity: number }): Barcode {
    ...
    async createBarcode(barcode: Barcode): Promise<Barcode> {
        const row = await this.prisma.barcode.create({
            data: { value: barcode.value, roomId: barcode.roomId, quantity: barcode.quantity },
        });
        return this.map(row);
    }
    ...
    async getRoomBarcodes(roomId: string, page: number, pageSize: number): Promise<Barcode[]> {
