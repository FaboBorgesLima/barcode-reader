...
export class BarcodeController {
  ...
  async create(args: string[]): Promise<void> {
    try {
      ...
      if (!room || !(await this.roomService.canViewRoom(room, userId)))
        return this.view.displayError('Room not found or access denied');
      const barcode = await this.barcodeService.createBarcode(roomId, value);
  ...
  async list(args: string[]): Promise<void> {
    try {
      ...
      if (!roomId) return this.usage('barcode list <roomId> [page] [size]');
  ...
  async update(args: string[]): Promise<void> {
    try {
      ...
      if (
        !room ||
        !(await this.barcodeService.canUpdateBarcode(barcode, room, userId))
      )
        return this.view.displayError('Room not found or access denied');
  ...
  async delete(args: string[]): Promise<void> {
    try {
      ...
      if (!id || !userId) return this.usage('barcode delete <id> <userId>');
      const barcode = await this.barcodeService.getBarcodeById(id);
      if (!barcode) return this.view.displayError('Barcode not found');
      const room = await this.roomService.getRoomById(barcode.roomId);
      if (
        !room ||
        !(await this.barcodeService.canDeleteBarcode(barcode, room, userId))
      )
        return this.view.displayError('Room not found or access denied');
  ...
  async export(args: string[]): Promise<void> {
    try {
      ...
      const barcodes = await this.barcodeService.exportAllBarcodes(roomId);
      this.view.displayList(barcodes);
    } catch (e) {
      this.view.displayError(e instanceof DomainError ? e.message : String(e));
    }
